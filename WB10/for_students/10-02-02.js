...

{
  ...
  cube2.stepWorld = function (delta, timeofday) {
    cubeTime += delta;
    //removed the time calculations and oscillations since we are now dealing with it in the
    //shader file - we still have to update time though since we are updating that global variable.
    //for some reason, even though I used the same calculation as what was here in the shader file for the 
    //oscillation effect, it was much slower, so I've changed cubeTime * 0.001 to cubeTime * 0.004.
    mat2.uniforms.time.value = cubeTime * 0.004; // pass in the time in seconds
  };
  world.add(cube2);
